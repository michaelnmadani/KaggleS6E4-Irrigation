...
# --- Parse args early so VERSION is available as a module-level constant ---
...
def _train_model_partial(model_type, seed, fold_start, fold_end, step_name):
    ...
    # Set the seed (check random_seed first for CatBoost's native param)
...
def step6_blend_and_finalize():
    """Multi-seed avg + L2 stacker + CatBoost-dominant blend + logit bias tuning + save submissions."""
    t0 = time.time()
    logger.info(f"=== STEP 6: V{VERSION} Multi-seed Avg + Stacker + Blend + Logit Bias + Finalize ===")
    ...
    # --- L2 Meta-Stacker: LogisticRegression on 9 OOF probability columns ---
    logger.info("Training L2 meta-stacker...")
    meta_X_train = np.hstack([lgbm_oof, xgb_oof, cat_oof])  # (n_train, 9)
    meta_X_test = np.hstack([lgbm_test, xgb_test, cat_test])  # (n_test, 9)
    ...
    # 5-fold CV to get stacker OOF predictions (avoid overfitting)
    meta_skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=RANDOM_STATE)
    stacker_oof = np.zeros_like(lgbm_oof)  # (n_train, 3)
    stacker_test_probs = np.zeros_like(lgbm_test)  # (n_test, 3)

    for fold_idx, (tr_idx, val_idx) in enumerate(meta_skf.split(meta_X_train, y_train)):
        lr = clone(stacker)
        lr.fit(meta_X_train[tr_idx], y_train[tr_idx])
        stacker_oof[val_idx] = lr.predict_proba(meta_X_train[val_idx])
        stacker_test_probs += lr.predict_proba(meta_X_test) / 5
        fold_ba = balanced_accuracy_score(y_train[val_idx], stacker_oof[val_idx].argmax(axis=1))
        logger.info(f"  Stacker fold {fold_idx+1}: BA = {fold_ba:.5f}")

    stacker_ba = balanced_accuracy_score(y_train, stacker_oof.argmax(axis=1))
    logger.info(f"  L2 meta-stacker overall BA: {stacker_ba:.5f}")

    results["l2_meta_stacker"] = {
        "fold_scores": [],
        "mean_balanced_accuracy": round(stacker_ba, 5),
        "metrics": compute_metrics(y_train, stacker_oof.argmax(axis=1), labels=[0, 1, 2]),
        "test_predictions": stacker_test_probs.argmax(axis=1),
        "test_probabilities": stacker_test_probs,
        "oof_probabilities": stacker_oof,
        "feature_importance": lgbm_s42["feature_importance"],
        "params": {"method": "l2_meta_stacker", "C": 1.0, "input_dim": 9},
    }
    ...
    logger.info(f"\nV{VERSION} Best: {best_model_name} = {best_score:.5f}")

    # --- Save submission files ---
    logger.info("Saving submission files...")

    submissions = {
        f"submission_v{VERSION}_catboost_dom.csv": "catboost_dominant_blend",
        f"submission_v{VERSION}_equal.csv": "equal_blend",
        f"submission_v{VERSION}_catboost.csv": "catboost",
        f"submission_v{VERSION}_lgbm.csv": "lightgbm",
        f"submission_v{VERSION}_stacker.csv": "l2_meta_stacker",
    }
    if "logit_bias_tuned" in results:
        submissions[f"submission_v{VERSION}_bias.csv"] = "logit_bias_tuned"
    ...
    save_submission(test_ids.tolist(), final_preds.tolist(), filename=f"submission_v{VERSION}.csv")
    ...
    logger.info(f"    L2 Stacker:    {stacker_ba:.5f}")
    ...
    for fname in submissions:
        logger.info(f"    {fname}")
    logger.info(f"    submission_v{VERSION}.csv (best: {best_model_name})")
